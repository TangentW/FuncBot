...
// MARK: - IO
public extension Message {
    static func filter<T>(for type: T.Type) -> (String) -> IO<T> where T: IncomingMessage {
    ...
    static func map<I>(_ fun: @escaping (I) -> Message.Outgoing?) -> (I) -> IO<Message.Outgoing> where I: IncomingMessage {
        return { incoming in
            IO { send in
                if let message = fun(incoming) {
                    send(message)
                }
            }
        }
    ...
    static func replyBind(refer: Bool = false, _ fun: @escaping (String) -> IO<String>?) -> (Message.Normal) -> IO<Message.Outgoing> {
        return { incoming in
            func createOutgoing(_ text: String) -> Message.Outgoing {
                let type: Message.Outgoing.MsgType
                if incoming.toUid != nil {
                    type = .P2P(to: incoming.uid)
                } else {
                    type = .channel(to: incoming.vchannelId)
                }
                return Outgoing(type: type, vchannelId: incoming.vchannelId, text: text, referKey: refer ? incoming.key : nil)
            }
            guard let next = fun(incoming.text) else { return IO.never }
            return createOutgoing <^> next
        }
    }
    
    static func replyMap(refer: Bool = false, _ fun: @escaping (String) -> String?) -> (Message.Normal) -> IO<Message.Outgoing> {
        return replyBind(refer: refer) {
            guard let value = fun($0) else { return nil }
